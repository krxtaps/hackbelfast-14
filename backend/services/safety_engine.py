...
from services.amenities.amenity_scoring import compute_amenity_signals, compute_sanctuary_signals
...
def _get_neighborhood_env_score(lat: float, lng: float, radius_m: float = 300.0) -> tuple[float, int]:
    """
...
async def get_combined_safety_score(lat: float, lng: float, check_time: str = None) -> Dict[str, Any]:
    """
    ...
    if nearest_street:
        ...
        env_explanations.append(f"Infrastructure: {abs(diff):.1f} points {standing} the Botanic average.")
    else:
    ...
    if nb_count > 1:
        env_explanations.append(f"Area Context: {abs(nb_diff):.1f} points {nb_standing} average regional safety.")
...
async def get_street_combined_score(street_id: str, check_time: str = None) -> Dict[str, Any]:
    """
    ...
    all_explanations.append(f"Infrastructure: {abs(env_diff):.1f} points {env_standing} the Botanic average.")
    if nb_count > 1:
        all_explanations.append(f"Area Context: {abs(nb_diff):.1f} points {nb_standing} average regional safety.")
